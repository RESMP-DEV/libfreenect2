# Review guidance for RESMP-DEV/libfreenect2
...
- Review RESMP-DEV/libfreenect2 as the source of truth for Open source drivers for the Kinect for Windows v2 device.
...
- For C/C++ changes, check bounds and integer overflow, ownership and lifetime, aliasing, alignment, thread safety, error propagation, compiler guards, and ABI/API compatibility.
- Tests must include edge sizes and failure cases on the supported toolchain or hardware, not only the happy-path build.
